...
<div class="card border-0 shadow-sm rounded-4">
    <div class="card-body p-4">
        ...
        <form method="post" action="<?= esc($formAction) . ($isPopup ? '?popup=1' : '') ?>" class="row g-3">
            ...
            <div class="col-md-4"><label class="form-label">Factura origen</label><select name="purchase_invoice_id" class="form-select" required><option value="">Seleccionar factura</option><?php foreach ($invoices as $invoice): ?><option value="<?= esc($invoice['id']) ?>"><?= esc($invoice['invoice_number'] . ' / ' . $invoice['supplier_name']) ?></option><?php endforeach; ?></select></div>
            ...
            <div class="col-md-6"><label class="form-label">Devolucion documentada (opcional)</label><select name="purchase_return_id" class="form-select"><option value="">Descuento independiente de devoluciones</option><?php foreach (($returns ?? []) as $return): ?><option value="<?= esc($return['id']) ?>"><?= esc($return['return_number']) ?></option><?php endforeach; ?></select><div class="form-text">Si documenta una devolucion ya registrada, seleccionarla para no descontarla dos veces. El monto debe coincidir.</div></div>
